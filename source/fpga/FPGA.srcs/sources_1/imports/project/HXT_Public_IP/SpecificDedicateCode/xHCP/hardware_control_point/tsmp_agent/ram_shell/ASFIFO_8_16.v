...
wire    [DataWidth-1:0]     wv_Ram_q_dealy;
always @(posedge ram_rdclock or posedge rd_aclr)//ParamSdpram only one cycle delay
    if(rd_aclr == 1'b1) ram_Ram_q <= {(DataWidth+1){1'b0}};
    else              ram_Ram_q <= wv_Ram_q_dealy;


ParamSdpram #(
    .ADDR_WIDTH(RAMAddWidth),
