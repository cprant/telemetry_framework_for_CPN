...
(
        ...
        ov_phase_cor           ,
        o_phase_cor_wr         ,
        ov_as6802_cycle_length ,//20220607
        ov_cycle_base_count    ,//
        iv_cycle_count         ,//
        ov_frequency_cor       ,
        o_frequency_cor_wr       
...
output reg [31:0]      ov_as6802_cycle_length;
output reg [15:0]      ov_cycle_base_count   ;
...
always @(posedge i_clk or negedge i_rst_n) begin
    if(i_rst_n == 1'b0)begin       
        ...
        ov_as6802_cycle_length<= 32'h0;
        ov_cycle_base_count   <= 16'h0;
        ov_phase_cor          <= 32'b0;
        o_phase_cor_wr        <= 1'b0 ;     
        ...
        if(i_stc_wr)begin
            ...
            else if((!i_stc_addr_fixed) && (iv_stc_addr == 19'd8))begin
                ov_as6802_cycle_length          <= iv_stc_wdata;      
                o_syn_clock_set_wr              <= 1'b0;             
            end  
            else if((!i_stc_addr_fixed) && (iv_stc_addr == 19'd9))begin
                ov_cycle_base_count             <= iv_stc_wdata;      
                o_syn_clock_set_wr              <= 1'b0;             
            end            
            else begin
                o_frequency_cor_wr              <= 1'b0; 
            ...
            if((!i_stc_addr_fixed) && (iv_stc_addr == 19'd0))begin
                o_stc_wr          <= 1'b1;
                ov_stc_rdata      <= {16'b0,ost_ver,osm_ver} ;
                ...
                o_stc_wr          <= 1'b1;
                ov_stc_rdata      <= ov_as6802_cycle_length;
                ov_stc_raddr      <= iv_stc_addr;
                o_stc_addr_fixed  <= i_stc_addr_fixed;
            end     
            else if((!i_stc_addr_fixed) && (iv_stc_addr == 19'd9))begin
                o_stc_wr          <= 1'b1;
                ov_stc_rdata      <= {iv_cycle_count,ov_cycle_base_count};
                ov_stc_raddr      <= iv_stc_addr;
                o_stc_addr_fixed  <= i_stc_addr_fixed;
            end
            else if((!i_stc_addr_fixed) && (iv_stc_addr == 19'd10))begin
                o_stc_wr          <= 1'b1;
                ov_stc_rdata      <= iv_syn_clk[63:32];
                ov_stc_raddr      <= iv_stc_addr;
                o_stc_addr_fixed  <= i_stc_addr_fixed;
            end     
            else if((!i_stc_addr_fixed) && (iv_stc_addr == 19'd11))begin
                o_stc_wr          <= 1'b1;
                ov_stc_rdata      <= iv_syn_clk[31:0];
                ov_stc_raddr      <= iv_stc_addr;
                o_stc_addr_fixed  <= i_stc_addr_fixed;
            end            
            else begin
