...
input                  i_nmu_fifo_empty     ;
output reg             o_nmu_fifo_rd        ;
input     [15:0]       iv_nmu_fifo_rdata    ;

input     [7:0]        iv_ack_fifo_usedw;
output reg             o_ack_fifo_rd    ;
input     [63:0]	   iv_ack_fifo_rdata;
    ...
    if(i_rst_n == 1'b0)begin 
        ...
        case(nmg_state)
            IDLE_S:begin//state report nmac pkt
                o_ack_fifo_rd             <= 1'b0;                 
                if((iv_ack_fifo_usedw != 8'b0)&&(!i_nmu_fifo_empty)&&(iv_ack_fifo_usedw == iv_nmu_fifo_rdata[7:0]))begin//receive rd_command_ack
                    rv_report_cnt       <= 8'h1;
                    rv_rd_state_num     <= iv_nmu_fifo_rdata;
            ...
            GENERATE_BASE_ADDR_S:begin
                ...
                case(rv_report_cnt[1:0])
                    2'd2:ov_data <= {1'b0,iv_ack_fifo_rdata[63:56]};
                    2'd3:ov_data <= {1'b0,iv_ack_fifo_rdata[55:48]}; 
                    2'd0:ov_data <= {1'b0,iv_ack_fifo_rdata[47:40]};
                    2'd1:ov_data <= {1'b0,iv_ack_fifo_rdata[39:32]};  
                    default:begin
                        ov_data  <= 9'd0;
                    end
                endcase
                ...
                if(rv_report_cnt[1:0] == 2'd1)begin
                    ...
                    if(rv_rd_state_num == 2'd1)begin//read all command ack of fifo 
                        if(rv_report_cnt < 8'd60)begin
                            ov_data    <= {1'b0,iv_ack_fifo_rdata[7:0]};                       
                            o_data_wr  <= 1'b1;
                            nmg_state  <= PAD_ZERO_S;
                        end 
                        else begin
                            ov_data      <= {1'b1,iv_ack_fifo_rdata[7:0]};                       
                            o_data_wr    <= 1'b1;
                            nmg_state    <= IDLE_S;
                        end
                        if(iv_ack_fifo_usedw == 8'd0)begin
                    ...
                    //generate data
                    case(rv_report_cnt[1:0])
                        8'd2:ov_data <= {1'b0,iv_ack_fifo_rdata[31:24]};
                        8'd3:ov_data <= {1'b0,iv_ack_fifo_rdata[23:16]};
                        8'd0:ov_data <= {1'b0,iv_ack_fifo_rdata[15:8]};
                        8'd1:ov_data <= {1'b0,iv_ack_fifo_rdata[7:0]};                       
                        default:begin
                            ov_data  <= 9'd0;
                        end
                    endcase                
