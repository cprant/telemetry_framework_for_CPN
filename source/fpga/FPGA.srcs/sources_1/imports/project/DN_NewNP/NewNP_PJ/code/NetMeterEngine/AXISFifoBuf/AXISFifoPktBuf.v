...
//限定FIFO深度至少是512(2的9次方)
localparam  DFIFO_DEEP = (DEEP_WIDTH <(13 - clogb2(DATA_BYTE))) ? (13 - clogb2(DATA_BYTE)) : DEEP_WIDTH,
            SFIFO_DEEP = DFIFO_DEEP-clogb2(((64/DATA_BYTE)<=1)?1:(64/DATA_BYTE));
/*//////////////////////////////////////////////////////////
                    中间变量声明区域
*///////////////////////////////////////////////////////////
...
reg  [  DATA_BYTE-1:0] tkeep_reg;//将tail_byte的尾部字节转换成尾部的tkeep
...
/*/////////////////////////////////////
            输入接口处理
*//////////////////////////////////////
//数据FIFO需要预留一个完整报文以便后续端口写入
//同时状态FIFO的深度为64，不能出现数据FIFO还未存满，状态FIFO溢出的情况
always @(posedge s_axis_clk or negedge s_axis_rst_n) begin
    if(s_axis_rst_n == 1'd0) begin
        s_axis_tready <= 1'd0;
    end
    else begin
        if(((&sfifo_wrusedw[SFIFO_DEEP-1:1]) == 1'b0) && 
           ((&dfifo_wrusedw[DFIFO_DEEP-1:(11-clogb2(DATA_BYTE))]) == 1'b0))
        //FIFO还能继续缓存一个最大完整报文
            s_axis_tready <= 1'd1;
        else if((s_axis_tvalid == 1'd1) && 
                (s_axis_tlast  == 1'd1))
        //FIFO已经存不下一个完整报文了,正在传输的报文刚传完，下个报文应该不让传了
            s_axis_tready <= 1'd0;
        else
        //此时FIFO无法存在完整报文，并且要么没有报文写入，要么正在写还没写完
...
always @(posedge m_axis_clk or negedge m_axis_rst_n) begin
    if(m_axis_rst_n == 1'b0) begin
        ...
        case(read_state)
            IDLE_S: begin//识别报文头部Metadata
                if((sfifo_rdempty == 1'b0) && (fifo_can_rd == 1'd1)) begin//报文可供读取
                    sfifo_rd   <= 1'b1;
                    dfifo_rd   <= 1'b1;
                    read_state <= SEND_S;
                end
            ...
            SEND_S: begin//对应输出接口有数据正在发送，而FIFO在上个周期也读出了1拍数据的情况
                if((dfifo_rd == 1'b1) && (dfifo_rdata[DFIFO_BIT-1] == 1'b1))begin//读到报文尾
                    if((sfifo_rdempty == 1'b0) && (fifo_can_rd == 1'd1)) begin//报文可供读取
                        sfifo_rd   <= 1'b1;
                        dfifo_rd   <= 1'b1;
                        read_state <= SEND_S;
...
//记录尾部字节tkeep的值，以便读出报文尾部后替换
always @(posedge m_axis_clk or negedge m_axis_rst_n) begin
    if(m_axis_rst_n == 1'b0)
        tkeep_reg <= {DATA_BYTE{1'b0}};
    else if(sfifo_rd == 1'b1)
        tkeep_reg <= sfifo_rdata;
    else
        tkeep_reg <= tkeep_reg;
end
...
always @* begin
    case({int_1_axis_tready,ext_0_axis_tvalid})
        ...
        //将RegBuf0的数据输出到RegBuf1，由RegBuf0接收FIFO数据
            ...
            int_0_axis_tkeep  = (dfifo_eop == 1'b1) ? tkeep_reg : {DATA_BYTE{1'b1}};
        ...
        2'b10: begin//两个Regbuf都能收，将数据送到离出口近的Buf1
            ...
            int_1_axis_tkeep  = (dfifo_eop == 1'b1) ? tkeep_reg : {DATA_BYTE{1'b1}};
